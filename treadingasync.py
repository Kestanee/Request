...
def get_data_sync(urls):
    ...
    et = time.time()
    elapsed_time = et - st
    print("Exulation time",elapsed_time,"seconds ")
    return  json_array
...
# get_data_sync(urls) #38.87
# get_data_threading(urls) #3.7
# asyncio.run(get_data_async_but_as_wrapper(urls)) #32.44 seconds
asyncio.run(get_data_async_concurrently(urls)) #3.83